...
const ShopForm = ({ initialValues }: { initialValues?: Shop }) => {
    ...
    ...(initialValues
      ? {
        defaultValues: {
          ...
          logo_image:
            initialValues.logo_image ??
            (initialValues.logo_image_url
              ? { thumbnail: initialValues.logo_image_url, original: initialValues.logo_image_url }
              : null),
          cover_image:
            initialValues.cover_image ??
            (initialValues.cover_image_url
              ? { thumbnail: initialValues.cover_image_url, original: initialValues.cover_image_url }
              : null),
          address: {
            street_address: initialValues.address?.street_address ?? '',
          ...
          balance: {
            payment_info: {
              name: initialValues.bank?.account_name ?? '',
              email: initialValues.raw_payload?.contact ?? '',
              bank: initialValues.bank?.iban ?? '',
              account: initialValues.bank?.account_number ?? '',
            },
          },
          settings: {
            ...
            notifications: initialValues.settings?.notifications ?? { email: '', enable: false },
            socials: initialValues.settings?.socials ?? [],
            isShopUnderMaintenance: initialValues.settings?.isShopUnderMaintenance ?? false,
            shopMaintenance: initialValues.settings?.shopMaintenance ?? {
              title: '',
              description: '',
              start: null,
              until: null,
            ...
            askForAQuote: initialValues.settings?.askForAQuote ?? { enable: false, quote: '', content: '' },
          },
        },
      }
      : {}),
  ...
  const onSubmit = (values: FormValues) => {
    ...
    const socials = values.settings.socials?.map((s) => ({
      icon: s.icon?.value ?? s.icon,
      url: s.url,
    })) ?? [];

    const shopMaintenance = values.settings.shopMaintenance
      ? {
        title: values.settings.shopMaintenance.title,
        description: values.settings.shopMaintenance.description,
        start: values.settings.shopMaintenance.start,
        until: values.settings.shopMaintenance.until,
        image: values.settings.shopMaintenance.image ?? undefined,
      }
      : undefined;

    const balance = values.balance.payment_info
      ? {
        account_name: values.balance.payment_info.name,
        email: values.balance.payment_info.email,
        bank: values.balance.payment_info.bank,
        account_number: values.balance.payment_info.account,
      }
      : undefined;

    const logo_image = values.logo_image
      ? {
        thumbnail: values.logo_image.thumbnail,
        original: values.logo_image.original,
      }
      : undefined;

    const cover_image = values.cover_image
      ? {
    ...
    const location = values.settings.location
      ? {
        lat: values.settings.location.lat,
        lng: values.settings.location.lng,
        formatted_address: values.settings.location?.formatted_address ?? undefined,
      }
      : undefined;
    ...
    const payload: Partial<ShopInput> = {
      ...
      logo_image,
      cover_image,
      website: values.settings.website ?? undefined,
      contact: values.settings.contact ?? undefined,
      address,
      location,
      balance,
      settings: {
        ...
        socials,
        isShopUnderMaintenance: values.settings.isShopUnderMaintenance,
        shopMaintenance,
        askForAQuote: values.settings.askForAQuote,
      },
    };
